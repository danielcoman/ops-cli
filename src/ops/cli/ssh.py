...
class SshParserConfig(SubParserConfig):
    ...
    def configure(self, parser):
        ...
        parser.add_argument('role', type=str, help='Server role to ssh to. Eg: dcs')
        parser.add_argument('-l', '--user', type=str, help='SSH User')
        parser.add_argument('--ssh-config', type=str, help='Ssh config file name in the ./ansible dir')
        parser.add_argument('--index', type=int, default=1, help='Index of the server from the group')
        parser.add_argument('ssh_opts', default=[], nargs='*', help='Manual ssh options')
        parser.add_argument('--tunnel', action="store_true", help="Use SSH tunnel, must pass --local and --remote")
        parser.add_argument('--ipaddress', action="store_true")
        parser.add_argument('--local', type=str, help="local [host-ip:]port for ssh proxy or ssh tunnel")
        parser.add_argument('--remote', type=int, help="remote port for ssh tunnel")
        parser.add_argument('--proxy', action="store_true", help="Use SSH proxy, must pass --local")
        parser.add_argument('--nossh', action="store_true", help="Port tunnel a machine that does not have SSH. Implies --ipaddress, and --tunnel; requires --local and --remote")
        parser.add_argument('--keygen', action='store_true', help='Create a ssh keys pair to use with this infrastructure')
...
class SshRunner(object):
    ...
    def __init__(self, cluster_config_path, cluster_config, ansible_inventory, ops_config, cluster_name, root_dir):
        """
    ...
    def run(self, args):
        if args.keygen:
            if self.cluster_config.has_ssh_keys:
                ...
                display('Trying to generate ssh keys in:\n{} and \n{}'.format(pub_key_file,prv_key_file))
        ...
        ssh_config = args.ssh_config or self.ops_config.get('ssh.config') or self.ansible_inventory.get_ssh_config()
        ...
        if args.proxy:
            command = "ssh -F %s %s -4 -N -D %s -f -o 'ExitOnForwardFailure yes'" % (ssh_config, ssh_host, args.local)
        ...
        if (os.path.isfile(sshpass_path) and os.access(sshpass_path, os.X_OK)):
            display("Using sshpass passwordless wrapper at %s" % (sshpass_path), color="green", stderr=True)
        else:
            display("sshpass passwordless wrapper NOT available in %s" % (sshpass_path), color="purple", stderr=True)

        display("SSH-ing to %s[%d] => %s" % (args.role, args.index, host.name), color="green", stderr=True)
