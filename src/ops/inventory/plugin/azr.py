...
class OpsAzureInventory(AzureInventory):
    ...
    def __init__(self,args={}):
        ...
        if self._dict_args['subscription_id'] is not None:
            self._dict_args.update({'subscription_id': str(self._dict_args['subscription_id'])})
            ...
            for host, hostvars in iteritems(self._inventory['_meta']['hostvars']):
                if ('role' in hostvars['tags'] and hostvars['tags']['role'] == 'bastion') or \
                  ('Adobe:Class' in hostvars['tags'] and hostvars['tags']['Adobe:Class'] == 'bastion'):
                    ...
                    self._inventory['_meta']['hostvars'][host]['ansible_ssh_host'] = bastions[hostvars['location']]+'--'+private_ip
    ...
    def _selected_machines(self, virtual_machines):
        ...
        for machine in virtual_machines:
            ...
            # filter only by tags
            if self.tags and not self.locations and self._tags_match(machine.tags, self.tags):
                selected_machines.append(machine)
            ...
            if self.locations and self.tags and self._tags_match(machine.tags, self.tags) and machine.location in self.locations:
                selected_machines.append(machine)
