...
- manage the password in vault with  ManagedVaultSecret (ex: generate it only if it's not already there)
...
class SimpleVault(object):
    ...
    def __init__(
        ...
        def try_reading_token_file():
            ret = None
            try:
                ret = open(os.path.expanduser('~/.vault-token'),"r").read().strip()
            except Exception:
                ret = None
                pass
            return ret
        def write_token(token=None):
            try:
                if token:
                    open(os.path.expanduser('~/.vault-token'),"w").write(token.strip())
            except Exception:
                display("Warning: could not persist token to ~/.vault-token",stderr=False, color='yellow')
                pass

        self.vault_addr = vault_addr or os.getenv('VAULT_ADDR',None) or "http://localhost:8200"
        #Actually maybe we should reconsider having a default vault addr. How often we will create infrastructures
        ...
        while not self.vault_conn.is_authenticated() and auto_prompt:
            ...
            display("Note: the default LDAP username (%s) can be overwritten with VAULT_USER" % self.vault_user, stderr=True,
                    color='yellow')
            display("      or to pass a token directly use VAULT_TOKEN", stderr=True, color='yellow')
            try:
                ...
                if self.ldap_attempts >= MAX_LDAP_ATTEMPTS:
                    display("FAILED authentication {} times".format(self.ldap_attempts),color='red')
                    raise e
                else:
                    pass
...
class ManagedVaultSecret(object):
    ...
    def __init__(
        ...
        if isinstance(policy,int):
            ...
            raise Exception("Incorrect policy specified. Use a number if unsure.")
        if path:
        ...
        if self.current_data.get(key):
            #something exists on that path, we assume the secret already exists and do nothing more
            pass
        else:
            #secret does not exist, we will generate it right now according to the desired policy
            ...
            if self.actual_policy['engine'] == 'passgen':
                try:
                    ...
                    display('MANAGED-SECRET: You need passgen python module in order to use the passgen engine.')
                    raise e
                try:
